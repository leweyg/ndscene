...
export type NDSceneDebugViewMode = "tree" | "list";

export type NDSceneDebugInspectorOptions = {
  title?: string;
  initiallyOpen?: boolean;
  initialViewMode?: NDSceneDebugViewMode;
};
...
export class NDSceneDebugInspector {
  ...
  readonly summaryLabelElement: HTMLSpanElement;
  readonly hintElement: HTMLSpanElement;
  readonly toolbarElement: HTMLDivElement;
  readonly metaElement: HTMLDivElement;
  readonly contentElement: HTMLDivElement;
  readonly treeButton: HTMLButtonElement;
  readonly listButton: HTMLButtonElement;

  runtime: NDSceneRuntime | null = null;
  viewMode: NDSceneDebugViewMode;

  constructor(options: NDSceneDebugInspectorOptions = {}) {
  ...
  setRuntime(runtime: NDSceneRuntime | null): void {
    this.runtime = runtime;
    this.render();
  }

  setViewMode(viewMode: NDSceneDebugViewMode): void {
  ...
  private createModeButton(label: string, viewMode: NDSceneDebugViewMode): HTMLButtonElement {
  ...
  private render(): void {
    ...
    this.metaElement.append(
      createChip(`${this.runtime.getOrderedCommits().length} commit${this.runtime.getOrderedCommits().length === 1 ? "" : "s"}`),
      createChip(`${activeScene.nodesByName.size} node${activeScene.nodesByName.size === 1 ? "" : "s"}`),
      createChip(`${activeScene.buffersByPath.size} buffer${activeScene.buffersByPath.size === 1 ? "" : "s"}`),
      createChip(`mode: ${this.viewMode}`),
    );

    this.contentElement.appendChild(this.renderRuntimeSections(this.runtime));
  }

  private renderRuntimeSections(runtime: NDSceneRuntime): HTMLElement {
  ...
  private renderTreeView(scene: NDSceneGraphRuntime): HTMLElement {
  ...
  private renderListView(scene: NDSceneGraphRuntime): HTMLElement {
    ...
    for (const node of nodes) {
      ...
      summary.append(
        createInlineLabel(node.name, "node"),
        createInlineMeta(node.parentName ? `parent ${node.parentName}` : "root"),
        createInlineMeta(`children ${node.edge.childNodeNames.length}`),
      );
      entry.appendChild(summary);
  ...
  private renderBufferSection(scene: NDSceneGraphRuntime): HTMLElement {
    ...
    for (const buffer of buffers) {
      ...
      const summary = document.createElement("summary");
      summary.append(
        createInlineLabel(buffer.path, "buffer"),
        createInlineMeta(buffer.format || "format unknown"),
        createInlineMeta(buffer.dataEncoded ? `${buffer.dataEncoded.length} bytes` : "external"),
      );
      entry.appendChild(summary);
...
export function createNDSceneDebugInspector(options: NDSceneDebugInspectorOptions = {}): NDSceneDebugInspector {
  return new NDSceneDebugInspector(options);
}

function ensureDebugWebStyles(): void {
...
function createInlineLabel(text: string, badge: string): DocumentFragment {
...
function createInlineMeta(text: string): HTMLSpanElement {
...
function createWarning(text: string): HTMLDivElement {
  const element = document.createElement("div");
...
function describeTensor(tensor?: NDTensorRuntime): string {
  if (!tensor) {
    return "none";
  }
